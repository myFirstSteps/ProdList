...
<c:set var="OriginProdIco" value="<img class='prodStatIcon' title='Ключевой продукт. Этот продукт виден всем пользователям и является праобразом для пользовательских продуктов.'height='16' width='16' src='resources/common_image/icons/Key.gif' alt='Ключевой'>"/>
...
<c:set var="UsersProdIco" value="<img class='prodStatIcon' title='Пользовательская копия продукта. Вы можете изменить некоторые свойства своей копии  ключевого продукта (задать цену, комментарий и т.д.).При этом нетронутые свойства будут соответствовать свойствам ключевого продукта.' height='16' width='16' src='resources/common_image/icons/Users.gif' alt='Пользовательский'>"/>
<c:set var="newProdIco" value="<img class='prodStatIcon' title='Новый продукт. Этот продукт еще не прошел проверку и не добавлен в общую базу. Пока он виден только Вам, но Вы можете с ним работать, добавлять его в списки и редактировать.' height='16' width='16' src='${icons}New.gif' alt='Новый'>"/>
<c:if test="${products[0] ne null}">
    ...
    <c:if test="${productsOnly eq null }">
        <script>
            /*Здесь очень много кривого, избыточного javascript кода. Его обязательно нужно переработать, но пока у этой задачи низкий приоритет.*/
